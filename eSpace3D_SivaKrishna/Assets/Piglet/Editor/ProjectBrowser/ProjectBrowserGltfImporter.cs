...
namespace Piglet
{
    ...
    public static class ProjectBrowserGltfImporter
    {
        ...
        /// menu.)
        /// </summary>
        private static ImportOptions _importOptions;
        ...
        private static void HandleDragAndDrop(string targetPath, string[] droppedPaths)
        {
            ...
            if (!_importOptions.EnableDragAndDropImport)
                return;
        ...
        private static IEnumerator<string> ImportCoroutine(List<string> gltfPaths, string baseImportDir)
        {
            foreach (string gltfPath in gltfPaths)
            {
                ...
                void OnProgress(ImportStep type, int count, int total)
                {
                ...
                if (_importOptions.SelectPrefabAfterImport)
                {
                    Selection.activeObject = importedPrefab;
                    yield return importPath;
                }

                if (_importOptions.AddPrefabToScene)
                {
                    ...
                    if (_importOptions.SelectPrefabInScene)
                    {
                ...
                if (_importOptions.OpenPrefabAfterImport)
                {
