...
namespace SFB
{
    public class StandaloneFileBrowserWindows : IStandaloneFileBrowser<string>
    {
        ...
        private static void ParseResults(char[] buffer, List<string> results, bool multiselect)
        {
            ...
            foreach (var c in buffer)
            {
                if (c == 0)
                {
                    ...
                    {
                        results.Add(multiselect && results.Count > 0 ? $"{results[0]}\\{currentString}" : currentString);
                    }
