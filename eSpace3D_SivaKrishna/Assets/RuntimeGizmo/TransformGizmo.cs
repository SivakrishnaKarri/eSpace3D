...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		public TransformType transformType = TransformType.None;
		...
		private bool showGizmoHandle = false;
		...
		void OnEnable()
		{
			MenuStrip.onClickUndo += Undo;
			MenuStrip.onClickRedo += Redo;
		...
		public bool IsAlreadySelected(Transform t)
		{
			return targetRoots.ContainsKey(t);
		}
		...
		public void Redo()
		{
			...
			DebugTexDisplay.instance.debugText.text = "Redo operation started..";
		...
		public void Move()
		{
			...
			DebugTexDisplay.instance.debugText.text = "Selected Move button..";
		...
		public void Rotate()
		{
			...
			showGizmoHandle = true;
			DebugTexDisplay.instance.debugText.text = "Rotate selected..";
		...
		public void Scale()
		{
			...
			showGizmoHandle = true;
			DebugTexDisplay.instance.debugText.text = "Scale is selected..";
			Debug.Log("Scale is selected");
		}

		public void Deselect(bool addCommand = true)
		{
			for (int i = 0; i < 2; i++)
			{
				showGizmoHandle = false;
				SetLines();
				ClearTargets(addCommand);
				transformType = TransformType.None;
			}

			SetSelectionFlag(false);
			DebugTexDisplay.instance.debugText.text = "Deselected..";
		...
		private bool selectionFlag = false;
		public void SetSelectionFlag(bool flag)
		{
			selectionFlag = flag;
		}
		...
		private void GetTarget()
		{
			if(nearAxis == Axis.None && Input.GetMouseButtonDown(0))
			{
				...
				if(Physics.Raycast(myCamera.ScreenPointToRay(Input.mousePosition), out hitInfo, Mathf.Infinity, selectionMask))
				{
					...
					else if(!isAdding && !isRemoving &&  !targetRoots.ContainsKey(target))
					{
		...
		public void Delete()
		{
			...
			foreach (Transform t in targetRoots.Keys)
				targets.Add(t.gameObject);

			UndoRedoManager.Insert(new DeleteCommand(this, targets.ToArray()));
			...
			Deselect(false);
			DebugTexDisplay.instance.debugText.text = "Delected successfully..";
		...
		{
			if(target.gameObject.GetComponent<Highlighter>() == null)
				target.gameObject.AddComponent<Highlighter>().ConstantOn(Color.red);
