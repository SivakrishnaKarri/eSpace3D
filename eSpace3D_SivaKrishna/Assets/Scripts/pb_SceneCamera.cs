...
namespace GILES
{
	...
	public class pb_SceneCamera : MonoBehaviour
	{
		...
		private bool panFlag, orbitFlag, zoomFlag;
		...
		{
			TransformGizmo.instance.SetSelectionFlag(true);
			panFlag = false;
			orbitFlag = false;
			zoomFlag = false;
		...
		private void Update()
		{
			if (Input.GetKeyDown(KeyCode.F))
			{
				if(TransformGizmo.instance.mainTargetRoot != null)
					Focus(TransformGizmo.instance.mainTargetRoot.gameObject);
			}
		}
		...
		void LateUpdate()
		{
			...
			if(zooming)
			{
				transform.position = ClampedCameraPos(Vector3.Lerp(previousPosition, targetPosition, (zoomProgress += Time.deltaTime) / zoomSpeed));
				if( Vector3.Distance(transform.position, targetPosition) < .1f) zooming = false;
			}

			if( ((Input.GetAxis(INPUT_MOUSE_SCROLLWHEEL) != 0f && zoomFlag) || (Input.GetMouseButton(RIGHT_MOUSE) && Input.GetKey(KeyCode.LeftAlt))) && CheckMouseOverGUI() && zoomFlag)
			{
				...
				transform.position = ClampedCameraPos(transform.localRotation * (Vector3.forward * -distanceToCamera) + pivot);
			}

			bool viewTool = true;// editor == null || editor.EnableCameraControls();
			...
			if (Input.GetMouseButton(RIGHT_MOUSE) && !Input.GetKey(KeyCode.LeftAlt) && Input.GetKey(KeyCode.LeftShift))
			{
				...
				transform.position = ClampedCameraPos(transform.position + transform.forward * speed * Input.GetAxis("Vertical"));
				transform.position = ClampedCameraPos(transform.position + transform.right * speed * Input.GetAxis("Horizontal"));
				/*try {
			 ...
			 * Orbit
			 */
			if((Input.GetKey(KeyCode.LeftAlt) && Input.GetMouseButton(LEFT_MOUSE)) || (orbitFlag  && Input.GetMouseButton(LEFT_MOUSE)))
			{
				panFlag = false;
				TransformGizmo.instance.SetSelectionFlag(false);
				...
				transform.localRotation = Quaternion.Euler( eulerRotation );
				transform.position = ClampedCameraPos(CalculateCameraPosition(pivot));
			}
			else
			/**
			 * Pan
			 */
			if(Input.GetMouseButton(MIDDLE_MOUSE) || panFlag/* || (Input.GetMouseButton(LEFT_MOUSE) && viewTool )*/ )
			{
				orbitFlag = false;
				TransformGizmo.instance.SetSelectionFlag(false);
				...
				transform.position = ClampedCameraPos(transform.position - transform.right * delta.x);
				transform.position = ClampedCameraPos(transform.position - transform.up * delta.y);
		...
		private Vector3 ClampedCameraPos(Vector3 pos)
		{
			pos.x = Mathf.Clamp(pos.x, -1.82f, 2.36f);
			pos.y = Mathf.Clamp(pos.y, 0.06f, 2.58f);
			pos.z = Mathf.Clamp(pos.z, -3.82f, 3.71f);
		...
		{
			instance.ZoomInternal(target, 1f);
		}
		...
		/*public void FocusOnSkeleton()
		{
			instance.ZoomInternal((hip1.position + hip2.position) / 2, 5f);
		}*/
		...
		private void ZoomInternal( Vector3 target, float distance )
		{
			...
			targetPosition = ClampedCameraPos(CalculateCameraPosition(pivot));
			zoomProgress = 0f;
			zooming = true;
		}
